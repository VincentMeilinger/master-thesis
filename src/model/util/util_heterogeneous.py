...
from src.model.util.util import contrastive_loss, custom_pair_collate, custom_triplet_collate, plot_loss, save_training_results
...
def hetero_graph_data_valid(data: Data, edge_spec: list):
    try:
...
class PairDataHarvester:
    ...
    def prepare_pairs(self):
        ...
        file_path = f'./data/valid_pairs/valid_pairs_{self.save_file_postfix}.json'
...
class TripletDataHarvester:
    def __init__(self, db: DatabaseWrapper, gs: GraphSampling, edge_spec: list, config: dict, valid_triplets_save_file: str = "valid_triplets"):
        self.db = db
        self.gs = gs
        self.triplets = []
        self.edge_spec = edge_spec
